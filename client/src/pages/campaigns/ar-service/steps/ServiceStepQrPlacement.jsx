...
const ServiceStepQrPlacement = ({ product }) => {
  const assetNoun = product?.assetNoun || 'card';
  const printLabel = product?.printAssetLabel || 'print-ready AR card';
  const { wizardData, updateWizardData, setWizardStep } = useArServiceRequestStore();
  ...
  const refreshPreview = useCallback(async () => {
    ...
    try {
      ...
      const prevStored = useArServiceRequestStore.getState().wizardData.compositedPreviewUrl;
  ...
  useEffect(() => {
    const t = window.setTimeout(() => refreshPreview(), 150);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h3 className="text-lg font-semibold text-[var(--text-primary)]">Mark QR placement</h3>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">
          Show us where the QR code should sit on your {assetNoun}. Our team will use this when building your {printLabel}.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,280px)_1fr]">
        <div className="order-2 flex flex-col gap-4 lg:order-1">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">Frame style</p>
          ...
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">Position</p>
        ...
        <div className="order-1 lg:order-2">
          ...
          >
            {previewUrl ? (
              <img src={previewUrl} alt={`${assetNoun} with QR preview`} className="block h-auto w-full select-none" draggable={false} />
            ) : (
              <div className="flex aspect-[1.6] items-center justify-center text-sm text-[var(--text-muted)]">
                Upload a {assetNoun} image first
              </div>
            )}
            {busy && (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <button type="button" onClick={() => setWizardStep(1)} className="inline-flex min-h-[44px] items-center gap-2 rounded-xl border border-[var(--border-color)] px-4 py-2.5 text-sm">
        ...
        >
          Next: Video &amp; links <ArrowRight size={15} />
        </button>
      </div>
    </div>
  );
...
export default ServiceStepQrPlacement;
